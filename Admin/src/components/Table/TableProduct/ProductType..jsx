...
export default function TableProductType(props) {
  ...
  const ListTitleHead = [
    ...
    { Name: "Mô tả" },
    { Name: "Ảnh" },
    userRole?.productType?.button?.delete && { Name: "Xóa" },
    userRole?.productType?.button?.update && { Name: "Cập nhật" },
    { Name: "Chi tiết" },
  ];
  ...
  function HandelUpdate(id) {
    userRole?.productType?.button?.update &&
      setBodyAdmin(<UpdateProductType id={id} />);
  }
  function HandelAdd() {
    userRole?.productType?.button?.add && setBodyAdmin(<AddProductType />);
  }
  return (
    <>
      {userRole?.productType?.button?.add && (
        <button
          type="button"
          onClick={() => HandelAdd()}
              ...
              <tbody>
                {List?.map((item, index) => (
                  <tr key={index} id={item?.Id}>
                    ...
                    {userRole?.productType?.button?.delete && (
                      <td>
                        <button
                          type="button"
