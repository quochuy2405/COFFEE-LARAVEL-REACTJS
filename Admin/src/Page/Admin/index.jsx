...
import { ROLES } from "../../app/roleRender";
...
function Admin() {
  ...
  const [fixTag, setFixTag] = useState(false);
  const Logout = () => {
    setOpen(true);
  };
  ...
  useEffect(() => {
    if (document.querySelectorAll(".tag_menu")[0]) {
      const tagActive = document.querySelectorAll(".tag_menu")[0];
      setFillerAdmin(tagActive.getAttribute("data-set"));
    }
    if (fixTag === false) {
      setFixTag(true);
    }
  ...
  useEffect(() => {
    ...
    var handleBody = () => {
      switch (fitterAdmin) {
        ...
        case "CUSTOMERS": {
          return userRole?.customer?.tag && <Customers />;
        }
        case "SALES": {
          return userRole?.employee?.tag && <Sales />;
        }
        case "ROLE": {
          return userRole?.role?.tag && <Role />;
        }
        case "PRODUCTTYPE": {
        ...
        case "REVENUE": {
          return userRole?.revenue?.tag && <Revenue />;
        }
        case "PRODUCT": {
          return userRole?.product?.tag && <Product />;
        }
        case "PRODUCTUPLOAD": {
          return userRole?.product?.add && <ProductUpload />;
        }
        case "SUPPLIER": {
          return userRole?.supplier?.tag && <Supplier />;
        }
        case "ADDSUPPLIER": {
          return userRole?.supplier?.add && <AddSupplier />;
        }
        case "STORE": {
          return userRole?.store?.tag && <Store />;
        }
        case "DISCOUNTS": {
    ...
    <div className="Admin">
      <div className="header">
        <div className="nav-top">
          ...
          <i onClick={Logout} className="fas fa-sign-out"></i>
        </div>
        ...
        <div className="main_menu" ref={ref}>
          ...
          {userRole?.mainTag?.CT && <div>
            ...
            <ul className="collapse" id="details">
              {userRole?.general?.tag && (
                <li className="tag_menu" data-set="FINANCE">
                  Kế toán
                </li>
              )}
              {userRole?.revenue?.tag && (
          ...
          {userRole?.mainTag?.SP && <div>
            ...
            >
              <i className="icon fad fa-store-alt"></i>Sản phẩm - Tin tức
              <i className=" fas fa-chevron-right"></i>
            </div>
            <ul className="collapse" id="products">
              {userRole?.product?.tag && (
                <li className="tag_menu" data-set="PRODUCT">
                  Danh sách
                </li>
              )}
              {userRole?.product?.add && (
          ...
          {userRole?.mainTag?.CD &&<div>
            ...
            <ul className="collapse" id="setting">
              ...
              {userRole?.productType?.tag && (
                <li className="tag_menu" data-set="PRODUCTTYPE">
                  Loại sản phẩm
                </li>
              )}
              {userRole?.promotion?.tag && (
